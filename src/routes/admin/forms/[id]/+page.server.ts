// +page.server.js - Updated to handle repeatable sections for household members
...
export async function load({ params, url }) {
    ...
    try {
        ...
        // Fetch sections for this form with repeatable section info
        const { data: sectionsData, error: sectionsError } = await supabase
            .from('form_section')
            .select('id, title, orderindex, formid, repeatable, section_type, max_instances')
        ...
        // Build the complete form structure with repeatable section support
        const mappedForm = {
            id: formBasic.id,
            ...
            sections: (sectionsData || []).map(section => {
                const baseSection = {
                    ...
                    maxInstances: section.max_instances || 1,
                ...
                // If section is repeatable, create initial instances (at least 1)
                        ...
                        instances: [
                            {
                                ...
                                fields: baseSection.fields.map(field => ({
                                    ...field,
                                    name: `${field.name}_instance_1`, // Unique name for this instance
                                    instanceId: 1
                                }))
                            }
                        ]
        ...
        console.log('Final form structure:', {
            ...
            totalFields: mappedForm.sections.reduce((acc, section) => {
                if (section.repeatable && 'instances' in section && Array.isArray(section.instances)) {
                    return acc + section.instances.reduce((instAcc, inst) => instAcc + inst.fields.length, 0);
                }
...
export const actions = {
    ...
    // Updated action to match frontend expectation: addHouseholdMemberSection
    addHouseholdMemberSection: async ({ request, params }) => {
        try {
            const data = await request.formData();
            const memberNumber = data.get('memberNumber');
            const sectionDataStr = data.get('sectionData');
            
            if (!memberNumber || !sectionDataStr) {
                return fail(400, { message: 'Missing required data' });
            }

            const sectionData = JSON.parse(sectionDataStr.toString());
            const formId = params.id; // Assuming your route is [id]

            console.log(`Adding household member ${memberNumber} with section:`, {
                sectionId: sectionData.id,
                title: sectionData.title,
                fieldCount: sectionData.fields?.length || 0
            });

            return {
                type: 'success',
                data: {
                    message: `Successfully added household member ${memberNumber}`,
                    sectionId: sectionData.id,
                    memberNumber: parseInt(memberNumber.toString())
                }
            };

        } catch (error) {
            console.error('Error adding household member section:', error);
            return fail(500, { 
                message: 'Failed to add household member section',
                error: error instanceof Error ? error.message : 'Unknown error'
            });
        }
    },

    // Updated action to match frontend expectation: removeHouseholdMemberSection
    removeHouseholdMemberSection: async ({ request, params }) => {
        try {
            const data = await request.formData();
            const sectionId = data.get('sectionId');
            
            if (!sectionId) {
                return fail(400, { message: 'Missing section ID' });
            }

            const formId = params.id; // Assuming your route is [id]

            console.log(`Removing household member section: ${sectionId} from form: ${formId}`);
            ...
            return {
                type: 'success',
                data: {
                    message: 'Successfully removed household member section',
                    removedSectionId: sectionId.toString()
                }
            };

        } catch (error) {
            console.error('Error removing household member section:', error);
            return fail(500, { 
                message: 'Failed to remove household member section',
                error: error instanceof Error ? error.message : 'Unknown error'
            });
        }
    },

    // Keep existing actions but rename to avoid confusion
    addHouseholdMember: async ({ request }) => {
        const data = await request.formData();
        const sectionId = data.get('sectionId');
        const instanceId = data.get('instanceId');

        if (!sectionId || !instanceId) {
            return fail(400, { message: 'Section ID and instance ID are required' });
        }

        try {
            // Fetch the section to get max instances limit
            ...
            if (sectionError) {
                ...
                return fail(500, { message: 'Failed to fetch section data' });
            }

            const maxInstances = sectionData.max_instances || 20;
            const newInstanceId = parseInt(instanceId.toString()) + 1;

            if (newInstanceId > maxInstances) {
                return fail(400, { message: `Maximum ${maxInstances} household members allowed` });
            }

            // Return success with new instance ID for frontend to handle
            return { 
                success: true, 
                newInstanceId,
                message: `Added household member ${newInstanceId}`
            };
        } catch (err) {
            console.error('Add household member error:', err);
            return fail(500, { message: 'Internal server error' });
        }
    },

    removeHouseholdMember: async ({ request }) => {
        const data = await request.formData();
        const sectionId = data.get('sectionId');
        const instanceId = data.get('instanceId');
        ...
        try {
            // For now, just return success - frontend will handle the removal
            // In a full implementation, you might want to delete saved data for this instance
            return { 
                success: true, 
                removedInstanceId: parseInt(instanceId.toString()),
                message: `Removed household member ${instanceId}`
            };
        } catch (err) {
    ...
    saveAllFields: async ({ request }) => {
        ...
        try {
            ...
            // Parse all field values from form data, including instance-specific fields
            for (const [key, value] of data.entries()) {
                if (key.startsWith('field_')) {
                    const fieldId = key.replace('field_', '');
                    fieldUpdates.push({ id: fieldId, value: value || '' });
                } else if (key.includes('_instance_') || key.includes('_member_')) {
                    // Handle instance-specific fields (e.g., name_instance_1, name_member_2)
                    fieldUpdates.push({ 
                        fieldKey: key,
                        value: value || '' 
                    });
